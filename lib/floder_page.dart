...
class _store_pageState extends State<store_page> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: [
           customTextField(hint: 'your name' , controller: namecontroller),
           SizedBox(height: 30,),
           customTextField(hint: 'your work'),
           SizedBox(height: 30,),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(onPressed: () async{
                Navigator.push(context, MaterialPageRoute(builder: (context)=>text_page()));
                try{
                  await authrepo().textclass(namecontroller.text, workcontroller.text);
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: CustomText(text: 'task added',
                   Color: AppColors.black,
                    fontSize: 18),));
                }catch (e){
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e')));
                }
              }, child:CustomText(text: 'Next page', Color: AppColors.black, fontSize: 16) ),
            )
        ],
      ),
    );
